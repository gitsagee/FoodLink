...
export const DonorDashboard = () => {
    ...
    const [foodForm, setFoodForm] = useState({
        name: '', type: 'fruits', quantity: '', expiryDate: '', price: '', description: '', imageUrl: ''
    });
    ...
    const handleAddFood = async (e) => {
        ...
        // Check access
        if (!user.access) { 
            showToast('You do not have access to add food', 'error');
            return; 
        }
        try {
            const response = await fetch(`${API_URL}/foods`, {
    ...
    const handleFoodChange = (e) => {
        setFoodForm({ ...foodForm, [e.target.name]: e.target.value, donor: user });
    };
